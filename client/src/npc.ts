...
function facingDirFromAngle(idx:number){ return idx===0 ? {dx:0,dz:1} : idx===1 ? {dx:1,dz:0} : idx===2 ? {dx:0,dz:-1} : {dx:-1,dz:0}; }
...
export function spawnWalker(){
  ...
  const choice = pickChar(); if (!choice || !choice.prefab) return;
  let baseObj: THREE.Object3D; let mixer: THREE.AnimationMixer;
  if (walkerPool.length){ const reused = walkerPool.pop()!; baseObj=reused.obj; mixer=reused.mixer; mixer.stopAllAction(); }
  else { baseObj = SkeletonUtils.clone(choice.prefab); mixer = new THREE.AnimationMixer(baseObj); }
  ...
  if (clip) { const action = mixer.clipAction(clip); action.reset(); }
  const speed = 0.6 + Math.random()*0.6;
  if (clip) { const action = mixer.clipAction(clip); action.timeScale = speed / BASE_WALK_ANIM_SPEED; action.play(); }
...
export function updateWalkers(dt:number){
  ...
  function setupTurn(w:Walker, newDir:THREE.Vector3){
    w.state='turn';
    w.turnT=0; w.turnDur=0.38 + Math.random()*0.18;
    w.startQuat = w.obj.quaternion.clone();
    const target = w.obj.position.clone().add(newDir);
    const tmp = new THREE.Object3D(); tmp.position.copy(w.obj.position); tmp.lookAt(target);
  ...
  function maybeIdle(w:Walker){
    if (w.state==='walk' && Math.random()<0.05){
      w.state='idle'; w.idleT=0; w.idleDur=1 + Math.random()*2.2;
  ...
  for (let i=walkers.length-1; i>=0; i--){
    ...
    if (w.state==='turn'){
      ...
      if (w.startQuat && w.endQuat){ w.obj.quaternion.copy(w.startQuat).slerp(w.endQuat, k); }
      if (k>=1){
        if (w.queuedDir){ w.dir.copy(w.queuedDir); w.axis = Math.abs(w.dir.x)>Math.abs(w.dir.z)?'x':'z'; w.perp.set(-w.dir.z,0,w.dir.x); }
        w.state='walk'; w.speed = w.baseSpeed;
    ...
    if (w.state==='walk'){
      ...
      if (Math.abs(localProgress) > ROAD_EDGE_MARGIN){
        ...
        if (roadExistsAt(nx, nz)) {
          ...
          const tc = (d:THREE.Vector3)=>({ cx: w.tileX + (Math.abs(d.x)>0.1?Math.sign(d.x)*CELL:0),
                                           cz: w.tileZ + (Math.abs(d.z)>0.1?Math.sign(d.z)*CELL:0) });
          if (roadExistsAt(tc(f).cx, tc(f).cz)) dirs.push(f);
          if (roadExistsAt(tc(l).cx, tc(l).cz)) dirs.push(l);
          if (roadExistsAt(tc(r).cx, tc(r).cz)) dirs.push(r);

          if (dirs.length>1){
            let chosen = f;
            const canLeft = dirs.some(d=>d===l), canRight = dirs.some(d=>d===r);
            const roll = Math.random();
            if (canLeft && roll<0.15) chosen = l; else if (canRight && roll>=0.15 && roll<0.30) chosen = r;
            if (!dirs.includes(f)) chosen = dirs[(Math.random()*dirs.length)|0];
            if (chosen !== f) setupTurn(w, chosen); else maybeIdle(w);
          } else if (dirs.length===1){
            if (dirs[0] !== f) setupTurn(w, dirs[0]); else maybeIdle(w);
          } else {
            const back = w.dir.clone().multiplyScalar(-1); setupTurn(w, back);
          }
        } else {
          const back = w.dir.clone().multiplyScalar(-1); setupTurn(w, back);
    ...
    w.life -= dt;
    if (w.life <= 0){ scene.remove(w.obj); walkers.splice(i,1); continue; }
  }
