...
export function setMode(m: CursorMode){ mode = m; }
export function getMode(){ return mode; }
export function setPiece(p: "I"|"L"|"X"){ piece = p; }
export function getPiece(){ return piece; }
export function incAngle(){ angleIndex = (angleIndex + 1) & 3; }
export function getAngle(){ return angleIndex; }
...
export function updateCursorOrient(){ if(cursor) cursor.rotation.y = ANG[angleIndex]; }
...
export function makePreview(){
  ...
  let key: ModelKey =
    ...
    mode === "well"     ? "WELL" :
    mode === "turbine"  ? "TURBINE" :
    mode === "sawmill"  ? "SAWMILL" :
    (piece as ModelKey);
  ...
  preview = obj; scene.add(preview!);
}
export function updatePreviewRotation(){ if(preview) preview.rotation.y = ANG[angleIndex]; }
...
export function keyFromCenter(wx:number,wz:number){ return `${wx}:${wz}`; }
...
export function setCursorVisible(v:boolean){ if(cursor) cursor.visible = v; }
export function setCursorPosition(x:number,z:number){ if(cursor) cursor.position.set(x, 0.001, z); }
export function setPreviewVisible(v:boolean){ if(preview) preview.visible = v; }
...
export function placeGeneric(
  ...
  if (money < cost) return { ok:false, err:"money" };
  const id = keyFromCenter(wx, wz);
  if (roads.has(id)||houses.has(id)||buildings.has(id)||wells.has(id)||turbines.has(id)||sawmills.has(id))
    return { ok:false, err:"occupied" };
  if (opts?.requireRoad && !hasAdjacentRoad(wx, wz)) return { ok:false, err:"no_road" };
